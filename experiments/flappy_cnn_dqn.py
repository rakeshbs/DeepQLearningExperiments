...
Compared to the MLP state-vector experiments:
  ...
  - 8 actors — M4 Max 128GB has enough unified memory; beyond ~8 actors the
    shared Metal GPU becomes the bottleneck (actors compete with the learner).
  ...
  - Ape-X paper hyperparams: per_alpha=0.6, per_beta=0.4, target_update=2500.

Train:  python -m experiments.flappy_cnn_dqn
Test:   python -m experiments.flappy_cnn_dqn --test [--best]
"""
...
def gap_proximity_shaper(env, reward, done):
    """
    Add a per-step bonus proportional to how close the bird is to the next
    pipe gap center. This makes rewards dense so the CNN gets a useful learning
    signal even in very short early episodes where the bird never reaches a pipe.

    The bonus is scaled to 0.5 max so it doesn't overwhelm the pipe reward (+10)
    or death penalty (-10). It's applied on top of the raw game reward, not
    instead of it — pipe clears and deaths still dominate once the policy matures.
    """
    next_pipe = None
    ...
    if next_pipe is not None:
        ...
        # proximity_bonus in [0, 0.5]: 0.5 when perfectly centred, 0 at screen edge
        proximity_bonus = 0.5 * max(0.0, 1.0 - dist / env.screen_height)
...
def make_algo():
    ...
    return DoubleDQN(DQNConfig(
        ...
        lr=1e-4,
        gamma=0.99,
        target_update_freq=2_500,  # Ape-X paper value (vs 1000 previously)
    ))
...
runner = ParallelRunner(
    ...
    config=RunnerConfig(
        buffer_size=100_000,  # doubled from 50k — 128GB unified memory can afford more diversity
        batch_size=32,
        train_start=5_000,
    ...
    num_actors=8,             # M4 Max 128GB can run 8 actors without starving the learner GPU
    epsilon_alpha=4.0,        # 7.0 is Ape-X paper value but assumes 360 actors; with 8 actors
    ...
    epsilon_base=0.4,         # Ape-X paper value
    epsilon_base_decay=1.0,   # Ape-X paper: actor epsilons are fixed, no decay
    epsilon_base_min=0.4,     # irrelevant when decay=1.0 but set to match epsilon_base
    per_alpha=0.6,            # Ape-X paper value (was 0.3)
    per_beta=0.4,             # Ape-X paper value (was 0.6)
